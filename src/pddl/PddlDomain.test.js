...
describe("addAction()", () => {
    ...
    test("precondition should be added as pddl predicate", () => {
        expect(JSON.stringify(lightDomain.predicates[0])).toMatch(
            JSON.stringify(["switched", "l"]),
        );
    });

    test("effect should be added as pddl predicate", () => {
        expect(JSON.stringify(lightDomain.predicates[1])).toEqual(
            JSON.stringify(["switched-on", "l"]),
        );
    });

    test("not effect should be added as pddl predicate", () => {
        expect(JSON.stringify(lightDomain.predicates[2])).toEqual(
            JSON.stringify(["switched-off", "l"]),
        );
    });
